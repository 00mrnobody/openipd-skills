...
> **我们开源了一套AI流程引擎：19个角色技能（含8个核心执行角色），把复杂产品开发流程结构化、协同化。**
...
其中，华为公开披露过其引入并持续实践IPD/IPD+体系；IBM也长期使用类似的集成产品开发方法。  
苹果、特斯拉等公司公开信息更多是各自NPI/阶段评审机制，而非明确对外宣称采用“IPD”这一术语。
...
OpenIPD是一套**开源的AI流程引擎**。当前仓库包含**19个角色技能**，其中先聚焦以下**8个核心执行角色**进行端到端协作：
...
**这还只是部分产出。** 当前仓库公开了该案例的**7份核心Demo文档**与**28张配图**，覆盖市场、技术、硬件、软件、制造、采购、财务等关键维度。
...
- **接入你自己的评审流程** —— 与内部系统对接
- **导出标准文档** —— 直接用于融资或研发
- **跳过长文档，直接出信息图/Slides** —— 用视觉化模式快速评审与汇报

**核心代码 + 19个角色技能（含8个核心执行角色）+ 公开示例Demo**，全部开放。

视觉化输出能力（Gemini渲染图/信息图/Slides）说明见：
